...
export default {
  async fetch(request, env) {
    ...
    // ---------------- API Endpoints ----------------
    if(path === "/simulate" && request.method==="POST") return new Response(JSON.stringify(await handleSimulate(request, env)), { headers: { ...cors, "Content-Type": "application/json" } });
    if(path === "/story" && request.method==="POST") return new Response(JSON.stringify(await handleStory(request)), { headers: { ...cors, "Content-Type": "application/json" } });
    if(path === "/save" && request.method==="POST") return await handleSave(request, env);

    // ---------------- Static Files ----------------
    if(path === "/" || path === "/index.html") {
      const html = await env.ASSETS.fetch(request);  // Serve HTML from KV / Assets
      return html;
    }
    if(path === "/script.js") {
      const js = await env.ASSETS.fetch(request); // Serve JS from KV / Assets
      return new Response(js.body, { headers: { "Content-Type": "application/javascript" } });
    }
...
// ---------------- Simulation Helpers ----------------
function sphereMass(diameterM, density = 3000) { const r=diameterM/2; return (4/3)*Math.PI*r**3*density; }
function kineticEnergyJoules(massKg, velocityMs) { return 0.5*massKg*velocityMs**2; }
function tntEquivalentMegatons(E){ return E/4.184e15; }
function craterDiameterEstimateM(E){ return 0.07*E**(1/3); }
function seismicMwEquivalent(E){ return (Math.log10(E)-5.24)/1.44; }
function tsunamiInitialWaveHeightM(E, waterDepthM=4000){ const scale=(E/1e15)**0.25; const depthFactor=Math.max(0.5,Math.min(2,4000/Math.max(1,waterDepthM))); return Math.max(0.01,Math.min(0.5*scale*depthFactor,200)); }

// ---------------- API Handlers ----------------
async function handleSimulate(req, env){
  const data = await req.json();
  const D=parseFloat(data.diameter_m||50), v=parseFloat(data.velocity_m_s||20000), rho=parseFloat(data.density||3000), def=parseFloat(data.deflection_m_s||0), water=parseFloat(data.water_depth_m||4000);
  const vEff=Math.max(0,v-def);
  const m=sphereMass(D,rho);
  const E=kineticEnergyJoules(m,vEff);
  const tnt=tntEquivalentMegatons(E);
  ...
  const tsunamiH=tsunamiInitialWaveHeightM(E,water);
  const tsunamiR=Math.min(5000,100*Math.pow(Math.max(0.001,tnt),0.25));
  return {input:data, results:{mass_kg:m, energy_joules:E, tnt_megatons:tnt, crater_diameter_m:crater, seismic_mw_equivalent:mw, tsunami_initial_height_m:tsunamiH, tsunami_radius_km:tsunamiR}};
}

async function handleStory(req){
  const s=await req.json(); const r=s.results||s;
  const lat=s.input?.impact_lat, lon=s.input?.impact_lon;
  const loc=lat&&lon?` at (${lat.toFixed(2)},${lon.toFixed(2)})`:'';
  return {story:`Impact${loc}: ${r.tnt_megatons.toFixed(2)} Mt TNT, crater ~${(r.crater_diameter_m/1000).toFixed(2)} km, magnitude ${r.seismic_mw_equivalent.toFixed(2)}, tsunami height ${r.tsunami_initial_height_m.toFixed(2)} m, reach ${r.tsunami_radius_km.toFixed(0)} km.`};
}
...
async function handleSave(req, env){
  try{
    ...
    const now=new Date();
    const key=`impact_${now.toISOString().replace(/[:.]/g,'-')}_${Math.random().toString(36).slice(2,8)}.json`;
    if(!env.R2_BUCKET) return new Response(JSON.stringify({error:"R2_BUCKET not configured"}),{status:500});
    await env.R2_BUCKET.put(key, JSON.stringify(body,null,2), {httpMetadata:{contentType:"application/json"}});
    return new Response(JSON.stringify({success:true,key}),{headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}});
